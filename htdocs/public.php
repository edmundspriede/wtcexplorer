...
require(CMS_LIBPATH.CMS_SYSPATH."d_public.inc");
//require(CMS_LIBPATH.CMS_SYSPATH."s_session.inc");

$sToken = getString("token");
$nModuleId = 500;

global $_REQUEST;

if (empty($sToken))
	halt_error("Sistēmas kļūda. Nov norādīta saite", true);

if (!d_public("S", array("token"=>$sToken), $aPublic))
	halt_error("Sistēmas kļūda. Nav saites ieraksta", true);

if (count($aPublic)!=1)
	halt_error("Sistēmas kļūda. Nekorekts saites ieraksts", true);

$aParams = explode ("&", $aPublic[0]["params"]);

foreach ($aParams as $k=>$v)
{
	$aSet = explode ("=", $v);
	$_REQUEST[$aSet[0]]=$aSet[1];
}


if (empty($nModuleId) || !s_module('S',array('id'=>$nModuleId),$aMod))
