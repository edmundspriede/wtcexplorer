...
class XPM3_SMTP extends XPM3_MIME {
	...
	static public function connect($host, $user = '', $pass = '', $port = self::PORT, $ssl = false, $timeout = self::TIMEOUT, $name = '', $exception = null) {
		try {
			...
			if (!is_int($port)) $errors[] = 'invalid port type';
			...
			if (!is_int($timeout)) $errors[] = 'invalid timeout type';
	...
	static public function sendtohost($host, $addrs, $from, $message, $name = '', $port = self::PORT, $timeout = self::TIMEOUT, $exception = null) {
		try {
			$errors = array();
			...
			if (count($errors) == 0) {
				...
				if ($fp) {
					if (!fwrite($fp, 'MAIL FROM:<'.$from.'>'.self::CRLF)) $res[13] = 'can not write';
