...
class XPM3_MIME extends XPM3_FUNC {
	...
	static public function decode_header($str) {
		try {
			if (is_string($str)) {
				$out = '';
				foreach (explode(self::CRLF."\t", $str) as $line) {
					if (count($exp = explode('?=', $line)) > 1) {
						$rep = '';
						foreach ($exp as $part) {
							$decode = false;
							if ($vhave = strstr($part, '=?')) {
								$spart = substr($vhave, 2);
								if ($code = stristr($spart, '?B?')) {
									$decode = base64_decode(substr($code, 2));
								} else if ($code = stristr($spart, '?Q?')) {
									if (self::is_printable($code)) $decode = quoted_printable_decode(substr($code, 2));
								}
								if ($decode) $decode = substr($part, 0, -strlen($vhave)).$decode;
							}
							$rep .= $decode ? $decode : $part.'?=';
						}
						$out .= substr($rep, 0, -2);
					} else $out .= $line;
				}
	...
	static public function compose($text, $html = false, $attach = false, $uniq = null) {
		try {
			$errors = array();
			if ($text) {
				if (!(is_array($text) && isset($text['name'], $text['disposition'], $text['header'], $text['content']) && is_string($text['name']) && is_string($text['header']) && is_string($text['content']))) $errors[] = 'invalid text message format';
			}
			if ($html) {
				if (!(is_array($html) && isset($html['name'], $html['disposition'], $html['header'], $html['content']) && is_string($html['name']) && is_string($html['header']) && is_string($html['content']))) $errors[] = 'invalid html message format';
			}
			if (!$text && !$html) $errors[] = 'message is not set';
			if ($attach) {
				if (is_array($attach) && count($attach) > 0) {
					foreach ($attach as $arr) {
						if (!(is_array($arr) && isset($arr['name'], $arr['disposition'], $arr['header'], $arr['content']) && is_string($arr['name']) && is_string($arr['header']) && is_string($arr['content']))) {
					...
					if ($text && $html) {
						if ($disp['inline'] && $disp['attachment']) {
							...
							$body .= '--'.$boundary1.self::CRLF.'Content-Type: multipart/related;'.self::CRLF."\t".'boundary="'.$boundary2.'"'.self::CRLF.self::CRLF.
								...
								'--'.$boundary3.'--'.self::CRLF;
							foreach ($attach as $desc) if ($desc['disposition'] == 'inline') $body .= '--'.$boundary2.self::CRLF.$desc['header'].self::CRLF.self::CRLF.$desc['content'];
							$body .= '--'.$boundary2.'--'.self::CRLF;
							foreach ($attach as $desc) if ($desc['disposition'] == 'attachment') $body .= '--'.$boundary1.self::CRLF.$desc['header'].self::CRLF.self::CRLF.$desc['content'];
							$body .= '--'.$boundary1.'--';
						} else if ($disp['inline']) {
							...
							$body .= '--'.$boundary1.self::CRLF.'Content-Type: multipart/alternative;'.self::CRLF."\t".'boundary="'.$boundary2.'"'.self::CRLF.self::CRLF.
								...
								'--'.$boundary2.'--'.self::CRLF;
							foreach ($attach as $desc) $body .= '--'.$boundary1.self::CRLF.$desc['header'].self::CRLF.self::CRLF.$desc['content'];
							$body .= '--'.$boundary1.'--';
		...
		try {
			...
			if (count($errors) == 0) {
				...
				if (strstr($body, '--'.$boundary.'--')) {
					...
					if (strstr($exp1[0], '--'.$boundary.self::CRLF)) {
						foreach (explode('--'.$boundary.self::CRLF, $exp1[0]) as $part) {
	...
	static public function split_compose($str) {
		try {
			...
			$multipart = false;
			$header = str_replace(';'.self::CRLF."\t", '; ', $exp1[0]);
			$header = str_replace(self::CRLF."\t", '', $header);
			if (!(count($exp2 = explode(self::CRLF, $header)) > 1)) throw new Exception('invalid 2 argument value', 1);
